...
from bigtenplus_plugin.xmltv_gen import generate_xmltv, get_channel_id, _build_channel_id, _format_xmltv_time
...
def test_upcoming_title_prefix():
    ...
    assert titles[0] == "UPCOMING: B1G+: Lakers vs Celtics"
    assert titles[1] == "B1G+: Lakers vs Celtics"
    assert titles[2] == "ENDED: B1G+: Lakers vs Celtics"
...
def test_subtitle_on_all_three_programmes():
    ...
    for prog in root.findall("programme"):
        ...
        assert prog.find("sub-title").text == "B1G+ • NBA"
