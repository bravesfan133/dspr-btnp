...
from .playlist import event_ends_after, extract_b1g_datetime
from .xmltv_gen import generate_xmltv, get_channel_id

logger = logging.getLogger(__name__)

EPG_UPLOAD_DIR = "/data/uploads/epgs"
EPG_FILENAME = "bigtenplus_epg.xml"
...
def remove_stale_channels(
    ...
    B1G+ index is reused by a current match is removed (replaced) regardless.
    ...
    for name_lower, channel in existing.items():
        ...
        if channel.channel_group_id != group.id:
            continue
        start = extract_b1g_datetime(channel.name)
        if start is None:
            continue
...
def sync_to_dispatcharr(
    ...
    epg_group_name = str(settings.get("epg_group_name", "B1G+")).strip() or "B1G+"
    channel_id_prefix = str(settings.get("channel_id_prefix", "B1G+")).strip() or "B1G+"
    channel_number_start = float(settings.get("channel_number_start", 900) or 900)
    source_name = str(settings.get("epg_source_name", "B1G+ EPG")).strip() or "B1G+ EPG"
